...
export function PublicBookingPanel({ studentSlug, treatmentSlug, citySlug }: { studentSlug: string; treatmentSlug: string; citySlug: string }) {
  ...
  async function book() {
    ...
    try {
      const response = await fetch(`/api/studenti/${encodeURIComponent(studentSlug)}/programari`, { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ slotId: selected.availabilitySlotId, offeringId: selected.offeringId, startsAt: selectedStart, patientNote }) });
