...
import { publicStudentAvailabilityHref } from "@/lib/public-students/public-student-booking-links";
...
export function PublicBookingPanel({ studentSlug, treatmentSlug, citySlug, locationRouteKey }: { studentSlug: string; treatmentSlug: string; citySlug: string; locationRouteKey?: string }) {
  ...
  const loadAvailability = useCallback(async (signal?: AbortSignal) => {
    setLoading(true);
    try {
      const response = await fetch(publicStudentAvailabilityHref(studentSlug, treatmentSlug, citySlug, locationRouteKey), { signal });
  ...
  }, [citySlug, studentSlug, treatmentSlug, locationRouteKey]);
  ...
  return <section aria-labelledby="booking-title" className="space-y-4">
    ...
    {patientState === "missing-birth-date" ? <Card className="border-primary/40"><CardContent className="space-y-4"><div className="flex gap-3"><ShieldCheck className="size-5" /><div><p className="font-medium">Completează data nașterii</p><p className="text-sm text-muted-foreground">O păstrăm în profilul privat; studentul vede numai vârsta calculată.</p></div></div><div className="max-w-sm space-y-2"><Label htmlFor="booking-birth-date">Data nașterii</Label><Input id="booking-birth-date" type="date" value={dateOfBirth} onChange={(event) => setDateOfBirth(event.target.value)} /></div>{error ? <p className="text-sm text-destructive">{error}</p> : null}<Button disabled={savingProfile || !dateOfBirth} onClick={() => void saveBirthDate()}>Salvează și continuă</Button></CardContent></Card> : null}
    {createdSlug ? <Card><CardContent className="space-y-3"><p className="flex items-center gap-2 font-medium"><CalendarCheck2 className="size-5" />Cererea a fost trimisă.</p><Button asChild><Link href={`/cont/programari/${createdSlug}`}>Vezi cererea</Link></Button></CardContent></Card> : null}
    {patientState !== "missing-birth-date" ? <Card className="overflow-hidden"><CardContent className="space-y-4">
      ...
      <div className="calendar-toolbar flex items-center gap-2"><Button variant="outline" size="icon" aria-label="Perioada anterioară" disabled={!canGoBack} onClick={() => calendarRef.current?.getApi().prev()}><ChevronLeft /></Button><Button variant="outline" className="min-w-32" onClick={() => calendarRef.current?.getApi().today()}>{navLabel}</Button><Button variant="outline" size="icon" aria-label="Perioada următoare" onClick={() => calendarRef.current?.getApi().next()}><ChevronRight /></Button><Select value={view} onValueChange={(next) => { if (next === "timeGridDay" || next === "timeGridWeek") calendarRef.current?.getApi().changeView(next); }}><SelectTrigger className="ml-auto w-32" aria-label="Vedere calendar"><SelectValue /></SelectTrigger><SelectContent><SelectItem value="timeGridDay">Zi</SelectItem><SelectItem value="timeGridWeek">Săptămână</SelectItem></SelectContent></Select></div>
      ...
      <div className="student-calendar booking-calendar min-w-0"><FullCalendar ref={calendarRef} plugins={[timeGridPlugin, interactionPlugin]} initialView={view} locale={roLocale} firstDay={1} headerToolbar={false} allDaySlot={false} slotMinTime="07:00:00" slotMaxTime="21:00:00" slotDuration="00:15:00" slotLabelInterval="01:00:00" slotLabelFormat={{ hour: "2-digit", minute: "2-digit", hour12: false }} eventTimeFormat={{ hour: "2-digit", minute: "2-digit", hour12: false }} nowIndicator validRange={{ start: localKey(new Date()) }} editable={false} selectable={false} eventClick={(click: EventClickArg) => { const slot = click.event.extendedProps.slot as PublicSlot; if (patientState === "ready" || patientState === "anonymous") { setSelectedId(slot.id); setSelectedStart(slot.startOptions[0] ?? ""); idempotencyKeyRef.current = crypto.randomUUID(); setBookingDialogOpen(true); setError(null); } }} eventContent={(content) => <BookingEventContent {...content} />} events={events} datesSet={datesChanged} dayHeaderFormat={{ weekday: "short", day: "numeric", month: "short" }} height="auto" expandRows scrollTime="07:00:00" scrollTimeReset={false} /></div>
    </CardContent></Card> : null}
    <Dialog open={bookingDialogOpen && Boolean(selected) && !createdSlug} onOpenChange={changeBookingDialog}>
      <DialogContent>
        <DialogHeader>
          <DialogTitle>Alege ora programării</DialogTitle>
          <DialogDescription>Selectează începutul potrivit din fereastra disponibilă și trimite cererea studentului.</DialogDescription>
        </DialogHeader>
        {selected ? (
          <div className="space-y-5">
            <PublicBookingSlotDetails slot={selected} />
            <div className="space-y-2">
              <Label htmlFor="booking-start">Ora de început</Label>
              {selected.startOptions.length === 1 ? (
                <p id="booking-start" className="rounded-md border px-3 py-2 text-sm font-medium">
                  {formatAppointmentInterval(
                    selected.startOptions[0],
                    new Date(new Date(selected.startOptions[0]).getTime() + selected.treatment.durationMinutes * 60_000).toISOString(),
                  )}
                </p>
              ) : (
                <Select value={selectedStart} onValueChange={(value) => { setSelectedStart(value); idempotencyKeyRef.current = crypto.randomUUID(); }}>
                  <SelectTrigger id="booking-start" className="w-full"><SelectValue /></SelectTrigger>
                  <SelectContent>
                    {selected.startOptions.map((start) => (
                      <SelectItem key={start} value={start}>
                        {formatAppointmentInterval(
                          start,
                          new Date(new Date(start).getTime() + selected.treatment.durationMinutes * 60_000).toISOString(),
                        )}
                      </SelectItem>
                    ))}
                  </SelectContent>
              ...
              <>
                <div className="space-y-2">
                  ...
                  <textarea
                    ...
                    maxLength={1000}
                    value={patientNote}
                    onChange={(event) => { setPatientNote(event.target.value); idempotencyKeyRef.current = crypto.randomUUID(); }}
                    className="min-h-24 w-full rounded-md border bg-transparent px-3 py-2 text-sm"
                  />
                </div>
                {error ? <p role="alert" className="text-sm text-destructive">{error}</p> : null}
                <DialogFooter>
                  <Button type="button" variant="outline" onClick={() => changeBookingDialog(false)}>Anulează</Button>
                  <Button disabled={submitting || !selectedStart} onClick={() => void book()}>
