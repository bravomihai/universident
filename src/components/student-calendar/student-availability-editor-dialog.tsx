...
export function StudentAvailabilityEditorDialog({ open, catalog, slot, range, pending, onOpenChange, onSave, onDelete, onAppointmentChange }: {
  ...
  const [selected, setSelected] = useState<Record<string, string>>(() => Object.fromEntries(slot?.offerings.filter((offering) => offering.studentTreatment.durationMinutes <= initialDurationMinutes).map((offering) => [offering.studentTreatment.id, offering.supervisor.id]) ?? []));
  ...
  function keepTreatmentsThatFit(nextStartTime: string, nextEndTime: string) {
    ...
    setSelected((current) => Object.fromEntries(Object.entries(current).filter(([treatmentId]) => {
      const treatment = catalog.treatments.find((item) => item.id === treatmentId);
      return treatment && treatment.durationMinutes <= nextDuration;
    })));
  }
  function changeStartTime(value: string) { setStartTime(value); keepTreatmentsThatFit(value, endTime); }
  function changeEndTime(value: string) { setEndTime(value); keepTreatmentsThatFit(startTime, value); }
  function toggleTreatment(id: string, checked: boolean) { const treatment = catalog.treatments.find((item) => item.id === id); if (checked && (!treatment || treatment.durationMinutes > selectedIntervalMinutes)) return; setSelected((current) => { const next = { ...current }; if (checked) next[id] = next[id] || catalog.supervisors[0]?.id || ""; else delete next[id]; return next; }); }
  ...
  return <Dialog open={open} onOpenChange={(next) => { if (!pending) onOpenChange(next); }}><DialogContent ref={dialogContentRef} className="max-h-[90vh] max-w-2xl overflow-y-auto focus:outline-none" onOpenAutoFocus={(event) => { event.preventDefault(); dialogContentRef.current?.focus({ preventScroll: true }); }}>
    ...
    <form className="space-y-5" onSubmit={submit}>
      ...
      <div className="space-y-3"><Label className="flex items-center gap-2"><Stethoscope className="size-4" />Tratamente și supervizori</Label>{catalog.treatments.map((treatment) => { const fits = selectedIntervalMinutes > 0 && treatment.durationMinutes <= selectedIntervalMinutes; const checked = fits && Object.hasOwn(selected, treatment.id); return <div key={treatment.id} className={`grid gap-3 rounded-xl border p-3 sm:grid-cols-[1fr_240px] sm:items-center ${fits ? "" : "border-amber-500/35 bg-amber-500/5 opacity-70"}`}><label className="flex items-center gap-3"><input type="checkbox" checked={checked} disabled={!fits || !editable || pending} onChange={(event) => toggleTreatment(treatment.id, event.target.checked)} /><span><span className="block font-medium">{treatment.treatment.name}</span><span className="text-xs text-muted-foreground">{treatment.durationMinutes} minute</span>{!fits ? <span className="block text-xs font-medium text-amber-700 dark:text-amber-300">{selectedIntervalMinutes > 0 ? `Prea lung pentru intervalul selectat (${selectedIntervalMinutes} minute disponibile).` : "Alege un interval valid pentru a activa tratamentul."}</span> : null}</span></label><Select value={checked ? selected[treatment.id] ?? "" : ""} disabled={!checked || !editable || pending} onValueChange={(supervisorId) => setSelected((current) => ({ ...current, [treatment.id]: supervisorId }))}><SelectTrigger className="w-full"><SelectValue placeholder="Alege supervizorul" /></SelectTrigger><SelectContent>{catalog.supervisors.map((item) => <SelectItem key={item.id} value={item.id}>{[item.academicTitle, item.fullName].filter(Boolean).join(" ")}</SelectItem>)}</SelectContent></Select></div>; })}</div>
