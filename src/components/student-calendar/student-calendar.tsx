...
import { CalendarCheck2, CheckCircle2, ChevronLeft, ChevronRight, Clock3, RotateCcw } from "lucide-react";
...
function EventContent({ event, timeText }: EventContentArg) { const slot = event.extendedProps.slot as CalendarSlot | undefined; if (!slot) return <div className="p-1 text-xs font-semibold">Selectat · {timeText}</div>; const selected = event.extendedProps.selected === true; const Icon = slot.appointments.some((item) => item.status === "PENDING") ? Clock3 : slot.appointments.some((item) => item.status === "CONFIRMED") ? CalendarCheck2 : slot.isException ? RotateCcw : CheckCircle2; return <div className="flex min-w-0 gap-1.5 p-1"><Icon className="mt-0.5 size-3 shrink-0" /><div className="min-w-0 leading-tight"><p className="truncate text-[11px] font-semibold">{slot.studentLocation.name}</p><p className="truncate text-[10px]">{slot.offerings.map((item) => item.studentTreatment.treatment.name).join(", ")}</p><p className="truncate text-[10px] opacity-75">{timeText}</p>{selected ? <p className="text-[10px] font-bold uppercase">Selectat</p> : null}</div></div>; }
...
export function StudentCalendar() {
  ...
  async function save(payload: { kind: "SINGLE" | "RECURRING"; studentLocationId: string; offerings: Array<{ studentTreatmentId: string; supervisorId: string }>; startsAt: Date; endsAt: Date; repeat: "none" | "weekly" | "biweekly"; appointmentReason: string | null }) {
    ...
    const body = selectedSlot ? { studentLocationId: payload.studentLocationId, offerings: payload.offerings, startsAt: payload.startsAt.toISOString(), endsAt: payload.endsAt.toISOString(), expectedVersion: selectedSlot.version, appointmentReason: payload.appointmentReason } : payload.kind === "SINGLE" ? { kind: "SINGLE", studentLocationId: payload.studentLocationId, offerings: payload.offerings, startsAt: payload.startsAt.toISOString(), endsAt: payload.endsAt.toISOString() } : { kind: "RECURRING", studentLocationId: payload.studentLocationId, offerings: payload.offerings, rule: { startsOn: parts, startMinuteOfDay, durationMinutes, weekdays: [weekdayEnum[new Date(`${parts}T12:00:00Z`).getUTCDay()]], intervalWeeks: payload.repeat === "weekly" ? 1 : 2, endMode: "NEVER" } };
    try { const response = await fetch(selectedSlot ? `/api/disponibilitate-student/${encodeURIComponent(selectedSlot.id)}` : "/api/disponibilitate-student", { method: selectedSlot ? "PATCH" : "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify(body) }); const result = await response.json(); if (!response.ok) return result.error ?? "Disponibilitatea nu a putut fi salvată."; setFeedback(selectedSlot ? "Apariția a fost actualizată." : payload.kind === "SINGLE" ? "Intervalul a fost adăugat." : "Seria a fost adăugată."); await load(); return null; } catch { return "Disponibilitatea nu a putut fi salvată."; } finally { setPending(false); }
  }
