...
import { bucharestPartsForInstant, utcInstantForBucharestLocal } from "@/lib/availability/bucharest-time";
...
function clampSelectionEndToStartDay(startsAt: Date, endsAt: Date) {
  if (localKey(startsAt) === localKey(endsAt)) return endsAt;
  const endParts = bucharestPartsForInstant(endsAt);
  return utcInstantForBucharestLocal(
    localKey(startsAt),
...
export function StudentCalendar() {
  ...
  const [loading, setLoading] = useState(true); const [pending, setPending] = useState(false); const [error, setError] = useTransientMessage(); const [feedback, setFeedback] = useState<string | null>(null);
  ...
  const [selectedSlotId, setSelectedSlotId] = useState<string | null>(null); const [draft, setDraft] = useState<DraftRange | null>(null); const [dragPreview, setDragPreview] = useState<DraftRange | null>(null); const [dialogOpen, setDialogOpen] = useState(false);
  ...
  const load = useCallback(async () => {
    ...
    try {
      ...
      const response = await fetch(`/api/disponibilitate-student?from=${encodeURIComponent(from)}&to=${encodeURIComponent(to)}`);
      const payload = await response.json();
      if (!response.ok) throw new Error(payload.error ?? "Calendarul nu a putut fi încărcat.");
  ...
  useEffect(() => { const timer = window.setTimeout(() => void load(), 0); const media = window.matchMedia("(max-width: 767px)"); const sync = () => { const next: CalendarView = media.matches ? "timeGridDay" : "timeGridWeek"; calendarRef.current?.getApi().changeView(next); setView(next); }; const viewTimer = window.setTimeout(sync, 0); media.addEventListener("change", sync); return () => { window.clearTimeout(timer); window.clearTimeout(viewTimer); media.removeEventListener("change", sync); }; }, [load]);
  const selectedSlot = slots.find((slot) => slot.id === selectedSlotId) ?? null;
  const events = useMemo(() => { const result = slots.map((slot) => eventFor(slot, slot.id === selectedSlotId)); const visibleDraft = draft ?? dragPreview; if (visibleDraft && !selectedSlotId) result.push({ id: "draft-selection", title: "Selectat", start: visibleDraft.startsAt, end: visibleDraft.endsAt, classNames: ["calendar-event-selected"], extendedProps: { selected: true } }); return result; }, [slots, selectedSlotId, draft, dragPreview]);
  ...
  function openRange(startsAt: Date, endsAt: Date) {
    const normalizedEndsAt = clampSelectionEndToStartDay(startsAt, endsAt);
    if (normalizedEndsAt <= startsAt) {
      setError("Alege un interval care se termină după ora de început.");
      return;
    }
    if (startsAt <= new Date()) {
      setError("Alege un interval viitor.");
      return;
    }
    setSelectedSlotId(null);
    setDraft({ startsAt, endsAt: normalizedEndsAt });
    setDialogOpen(true);
    setFeedback(null);
  }
  function previewRange(startsAt: Date, endsAt: Date) {
    const normalizedEndsAt = clampSelectionEndToStartDay(startsAt, endsAt);
    const valid = startsAt > new Date() && normalizedEndsAt > startsAt;
    setDragPreview((current) => {
      if (!valid) return null;
      if (current?.startsAt.getTime() === startsAt.getTime() && current.endsAt.getTime() === normalizedEndsAt.getTime()) return current;
      return { startsAt, endsAt: normalizedEndsAt };
    });
    return valid;
  }
  async function save(payload: { kind: "SINGLE" | "RECURRING"; scope: "OCCURRENCE" | "SERIES"; studentLocationId: string; offerings: Array<{ studentTreatmentId: string; supervisorId: string }>; startsAt: Date; endsAt: Date; repeat: "none" | "weekly" | "biweekly"; recurrenceEndMode: "COUNT" | "UNTIL"; occurrenceCount: number | null; endsOn: string | null; appointmentReason: string | null }) {
    ...
    const parts = localKey(payload.startsAt); const startParts = bucharestPartsForInstant(payload.startsAt); const endParts = bucharestPartsForInstant(payload.endsAt); const startMinuteOfDay = startParts.hour * 60 + startParts.minute; const durationMinutes = endParts.hour * 60 + endParts.minute - startMinuteOfDay;
  ...
  return <div className="space-y-5">
    ...
    <Card className="overflow-hidden"><CardContent className="space-y-4"><div className="calendar-toolbar flex items-center gap-2"><Button variant="outline" size="icon" aria-label="Perioada anterioară" disabled={!canGoBack} onClick={() => calendarRef.current?.getApi().prev()}><ChevronLeft /></Button><Button variant="outline" className="min-w-32" onClick={() => calendarRef.current?.getApi().today()}>{navLabel}</Button><Button variant="outline" size="icon" aria-label="Perioada următoare" onClick={() => calendarRef.current?.getApi().next()}><ChevronRight /></Button><Select value={view} onValueChange={(next) => { if (next === "timeGridDay" || next === "timeGridWeek") calendarRef.current?.getApi().changeView(next); }}><SelectTrigger className="ml-auto w-32" aria-label="Vedere calendar"><SelectValue /></SelectTrigger><SelectContent><SelectItem value="timeGridDay">Zi</SelectItem><SelectItem value="timeGridWeek">Săptămână</SelectItem></SelectContent></Select></div>{loading ? <p className="text-sm text-muted-foreground">Se încarcă calendarul…</p> : null}<div className="student-calendar availability-calendar min-w-0"><FullCalendar ref={calendarRef} plugins={[timeGridPlugin, interactionPlugin]} initialView="timeGridWeek" locale={roLocale} firstDay={1} headerToolbar={false} allDaySlot={false} slotMinTime="07:00:00" slotMaxTime="21:00:00" slotDuration="00:15:00" slotLabelInterval="01:00:00" slotLabelFormat={{ hour: "2-digit", minute: "2-digit", hour12: false }} eventTimeFormat={{ hour: "2-digit", minute: "2-digit", hour12: false }} nowIndicator validRange={{ start: localKey(new Date()) }} selectable={ready} selectMirror selectAllow={(selection) => previewRange(selection.start, selection.end)} select={(selection: DateSelectArg) => { setDragPreview(null); openRange(selection.start, selection.end); selection.view.calendar.unselect(); }} unselect={() => setDragPreview(null)} dateClick={(click: DateClickArg) => { if (ready) openRange(click.date, new Date(click.date.getTime() + 60 * 60_000)); }} editable={false} eventClick={(click: EventClickArg) => { const slot = click.event.extendedProps.slot as CalendarSlot | undefined; if (!slot) return; setDraft(null); setSelectedSlotId(slot.id); setDialogOpen(true); }} eventContent={(content) => <EventContent {...content} />} events={events} datesSet={datesChanged} dayHeaderFormat={{ weekday: "short", day: "numeric", month: "short" }} height="auto" expandRows scrollTime="07:00:00" scrollTimeReset={false} /></div></CardContent></Card>
