...
const genericResponse = {
  message:
    "Dacă adresa corespunde unui cont, vei primi un email cu instrucțiuni pentru resetarea parolei.",
};
...
export async function POST(request: Request) {
  ...
  const authRequest = new Request(
    new URL("/api/auth/request-password-reset", request.url),
    {
      method: "POST",
      headers: authRequestHeaders,
      body: JSON.stringify({
        email,
        redirectTo: "/resetare-parola",
      }),
    },
  );
